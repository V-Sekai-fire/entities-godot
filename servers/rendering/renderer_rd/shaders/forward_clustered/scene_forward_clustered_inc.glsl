...
layout(set = 1, binding = 39) uniform sampler3D oit_transmittance;

layout(set = 1, binding = 40, std140) uniform OITParams {
	vec4 slice_curve;
	uvec4 froxel_dims;
}
oit_params;

layout(set = 1, binding = 41, std430) buffer restrict OITExtinction {
	uint data[];
}
oit_extinction;
