...
`VideoStreamCineForm` reads the files this module writes. Assign one to a `VideoStreamPlayer`
and it plays like any other stream.

The loader claims `.mkv` as well as `.cfhd`, so it checks that the file really carries
CineForm before accepting it: the video track has to be `V_MS/VFW/FOURCC` with a `CFHD`
fourcc in its `BITMAPINFOHEADER`. A Matroska holding anything else is left to another loader.

Verified against FFmpeg's own `cfhd` decoder on the same file, every pixel agrees to within
one code value out of 255.
