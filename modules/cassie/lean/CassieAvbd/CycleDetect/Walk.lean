...
def nextEdgePort (g : Graph) (nid : NodeId) (incoming : EdgeId)
  ...
  if idx ≥ eids.size then
    ...
    return NodeAugment.getInPlane g nid incoming transportedNormal
      (¬ reversed) eids
  -- Unity smooth-node step: `reversed` toggles `GetPrevious` (CW, -1) vs
  -- `GetNext` (CCW, +1) around the CCW-sorted neighbor ring. Kept in sync
  -- with the sharp branch above, where `wantNext = ¬ reversed`.
